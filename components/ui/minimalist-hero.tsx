    ...
    >
      ...
      <div className="relative grid w-full max-w-7xl grow grid-cols-1 items-center md:grid-cols-3">
        ...
        <motion.div
          ...
          className="z-20 order-2 md:order-1 flex flex-col items-center justify-center text-center md:items-end md:justify-start md:text-right md:pr-20 lg:pr-24"
        >
          <div className="flex flex-col gap-1 mb-6 space-y-1">
            {Array.isArray(mainText) ? (
              mainText.map((line, index) => {
                const offsets = [
                  "md:-translate-x-6",  // Line 1
                  "md:-translate-x-10", // Line 2
                  "md:-translate-x-10", 
                  "md:-translate-x-8", 
                ];
                ...
                const highlightText = (text: string) => {
                  ...
                  return parts.map((part, i) => 
                    part.toLowerCase() === "#1" || part.toLowerCase() === "marketing agency" ? (
                      <span key={i} className="text-[#E5B401]">{part}</span>
                    ) : (
                      part
                    )
                  );
        ...
        <motion.div
          ...
          className="z-20 order-3 flex flex-col items-center justify-center text-center md:items-start md:justify-start md:text-left"
        >
          <h1 className="flex flex-col gap-1 text-5xl font-black uppercase tracking-tight text-foreground md:text-6xl lg:text-7xl xl:text-8xl space-y-4">
            {overlayText.map((line, index) => {
              ...
              const offsets = [ 
                "md:translate-x-14",  // Line 1: WE
                "md:translate-x-20", // Line 2: CREATE (push out)
                "md:translate-x-20", // Line 3: CATEGORY (peak of curve)
                "md:translate-x-16", // Line 4: LEADERS (pull back in)
              ];   
              const offsetClass = offsets[index] || 'md:translate-x-0';
